...
public class DataModel {
    ...
    public DataModel(int numberOfPoints, double timeStep, IntegrationMethods method,
                     double xMin, double xMax, double yMin, double yMax) {
        //Here listOfPoints gets assigned
        double angle = PI/20;
        pointsArray = buildNewPoints(numberOfPoints, 100, timeStep, angle, angle, angle, method, xMin, xMax, yMin, yMax);
    }

    private PointsArray buildNewPoints (int numOfPoints, double time, double timeStep,
        ...
        double lambda0;
        lambda0 = cPh0*cPs0*cTh0 + sPh0*sPs0*sTh0;
        double lambda1;
        lambda1 = sPh0*cPs0*cTh0 - cPh0*sPs0*sTh0;
        double lambda2;
        lambda2 = cPh0*cPs0*sTh0 + sPh0*sPs0*cTh0;
        double lambda3;
        lambda3 = cPh0*sPs0*cTh0 - sPh0*cPs0*sTh0;
        ...
        /*for (int i = 1; i <= numOfPoints; i++) {
            eps = 1.0 * i / (numOfPoints);
            //eps = i;
            comboArray.y_val[i-1] = eps;
            if(comboArray.y_val[i-1]==0) comboArray.y_val[i] = 0.000001;
        }
        for (int j = 0; j < numOfPoints; j++) {
            del = 1 + 1.0 * j / (numOfPoints - 1);
            //del = j;
            comboArray.x_val[j] = del;
        }*/
        ...
        for (int i = 0; i < numOfPoints; i++) {
            for (int j = 0; j < numOfPoints; j++) {
                ...
                for (int t = 1; t <= time/timeStep; t++) {
                    ...
                    //out.println(comboArray.y_val[i]);
                    //out.println(comboArray.x_val[j]);
